...
public class VRegRes extends JPanel implements IPanels{
	...
	public Restaurante getRest() {
		...
		String nombre = txtNom.getText() , region = (String)cmbReg.getSelectedItem(), ciudad = txtCiu.getText()
				, direccion = txtDirec.getText(), cocina = (String)cmbCocina.getSelectedItem(), telefono = txtTel.getText(), web = txtWeb.getText();
		double precioMin = Double.parseDouble(txtMin.getText()), precioMax = Double.parseDouble(txtMax.getText());
		
		if(nombre.isEmpty()) {
			showErr("Debe introducir el nombre del restaurante");
		}
		
		return new Restaurante(distincion, nombre, region, ciudad, direccion, cocina,telefono, web, precioMin, precioMax);
	}
